...
sigma = Symbol("sigma") #
gaussian = exp(-(x**2 + 4 * y**2) / (8 * sigma**2)) / (4 * np.pi * sigma**2)
...
ricker2D_func = lambdify( [x, y, sigma], ricker2D_expr)

print(ricker2D_expr)

sigma = 0.01
ricker2D = ricker2D_func(xx, yy, sigma)
...
frq_m = 1 / (np.sqrt(2) * np.pi * 2 * sigma)
